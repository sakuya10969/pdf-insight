...
### ストレージ管理
- PDFファイルおよび解析結果JSONはローカルファイルシステムで管理
- RDB・クラウドストレージ（S3, GCS等）は使用しない
- doc_id（UUID）単位でPDFとJSONを紐付け管理
- JSONはPDF解析結果（チャンク情報）のキャッシュとして保持し、再embeddingやデバッグに再利用
- ファイル削除・更新はdoc_id単位で行う
